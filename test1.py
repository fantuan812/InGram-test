...
def test(epoch):
    ...
    if not args.best:
        my_model.load_state_dict(torch.load(f"ckpt/{args.exp}/{args.data_name}/{file_format}_{target_epoch}.ckpt")["model_state_dict"])

    else:
        my_model.load_state_dict(torch.load(f"ckpt/{args.exp}/{args.data_name}/{file_format}_best.ckpt")["model_state_dict"])
    ...
    if not args.best:
        test_init_emb_ent = torch.load(f"ckpt/{args.exp}/{args.data_name}/{file_format}_{target_epoch}.ckpt")["inf_emb_ent"]
        test_init_emb_rel = torch.load(f"ckpt/{args.exp}/{args.data_name}/{file_format}_{target_epoch}.ckpt")["inf_emb_rel"]
    else:
        test_init_emb_ent = torch.load(f"ckpt/{args.exp}/{args.data_name}/{file_format}_best.ckpt")["inf_emb_ent"]
        test_init_emb_rel = torch.load(f"ckpt/{args.exp}/{args.data_name}/{file_format}_best.ckpt")["inf_emb_rel"]
    ...
    mrr=evaluates(my_model, test, args.target_epoch-1, test_init_emb_ent, test_init_emb_rel, test_relation_triplets)
