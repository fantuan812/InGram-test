...
for i in range(num):
    ...
    if not args.single:
        if not args.best:
            test_init_emb_ent = torch.load(f"ckpt/{args.exp}/{args.data_name}/{str(i) }/best.ckpt")["inf_emb_ent"]
            test_init_emb_rel = torch.load(f"ckpt/{args.exp}/{args.data_name}/{str(i) }/best.ckpt")["inf_emb_rel"]
        else:
