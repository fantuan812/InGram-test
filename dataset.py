...
class TestData():
	def __init__(self, path,data_type = "valid"):
		...
		self.ent2id = None
		self.rel2id = None
		self.triplets,self.filter_dict= self.read_triplet(self.path + self.data_type + ".txt")
		# 计算三元组、实体和关系的数量
		self.num_triplets = len(self.triplets) # 三元组的总数
		self.num_ent, self.num_rel = len(self.ent2id), len(self.rel2id) # 实体和关系的数量
		# self.sr2o = ddict(set)
	...
	def split_transductive(self, p):
		...
		return np.array(msg), np.array(sup)
	def loads(self,path):
		ent2id=dict()
		rel2id=dict()
		with open(path.replace(self.data_type + ".txt",'')+ "entities.dict", 'r') as f:
			for line in f.readlines():
				lines = line.strip().split('\t')
				if(len(lines)<2):
					continue
					...
					continue
				ent2id[entity] = int(id)
		with open(path.replace(self.data_type + ".txt",'')+"relations.dict", 'r') as f:
			for line in f.readlines():
				lines = line.strip().split('\t')
				if(len(lines)<2):
					continue
